	...
	<div id="wrapper">
		...
		<div id="main">
			<div class="inner">
				...
				<div class="">
					<span style="font-size:20px; color: black;"><c:out value="인기 게시판"/></span>
					&nbsp;&nbsp; <c:out value="[" /> &nbsp;
					<span><c:out value="높은 조회수를 기록한 게시글 게시판 입니다"/></span>
					&nbsp; <c:out value="]" />
				</div>
				<br/>
				...
				<div class="table-wrapper" id="table">
					<table class="table table-striped table-sm">
						...
						<tbody>
							<c:choose>
								<c:when test="${empty viewBestBoardList }">
									<tr>
										<td colspan="5" align="center">데이터가 없습니다</td>
									</tr>
								</c:when>
								<c:when test="${not empty viewBestBoardList}">
					...
					<ul class="pagination" style="margin-top: 10px">
						...
						<c:forEach begin="${pagination.startPage}" end="${pagination.endPage}" var="idx">
							<li>
								...
								href="${pageContext.request.contextPath}/board/getViewBestBoardList${pagination.makeSearch(idx)}">${idx}</a>
							</li>
						</c:forEach>
...
<script>
	...
	function setPageCnt() {
		...
		$selectPageCnt.on('change', function(){
			location.href = "${pageContext.request.contextPath}/board/getViewBestBoardList?bg_no=${searchCriteria.bg_no}" + "&page=" + page + "&listSize=" + $selectPageCnt.val();
		});
	}
	...
	function setSearchPage() {
		...
		$(document).on('click', '#btnSearch', function(){
			...
			var url = "${pageContext.request.contextPath}/board/getViewBestBoardList${pagination.makeQuery(1)}"
